...
std::istream& Complex::readFrom(std::istream& istrm) {
  ...
  char rightBrace(0);
  istrm >> leftBrace >> real >> comma >> space >> imaginary >> rightBrace;
  if (istrm.good())
  {
    if ((Complex::leftBrace == leftBrace) && (Complex::separator_comma == comma) && (Complex::separator_space == space) && (Complex::rightBrace == rightBrace))
    {
      re = real;
      im = imaginary;
    }
